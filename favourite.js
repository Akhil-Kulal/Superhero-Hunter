...
function removeFromFavourite(element) {
  ...
  if (
    confirm(
      "Remove " +
        element.parentElement.previousSibling.firstChild.innerHTML +
        " from favourites?"
    )
  ) {
  ...
  for (let i = 0; i < storedCharactersArray.length; i++) {
    console.log(
      "removeFromFavourite Condition : ",
      storedCharactersArray[i].id + " == " + characterId
    );
    ...
    if (storedCharactersArray[i].id == characterId) {
      ...
      alert(
        element.parentElement.previousSibling.firstChild.innerHTML +
          " has been removed from favourites."
      );
    }
  }
}
